...
namespace WEITUO
{
    ...
    public class dailiren
    {
        ...
        public void beidailiren(string houseName, string address)
        {
      ...
      void Speak()
       ...
       public void SpeakFun()
        ...
        {
            //第一种方法。反射可以看清一个对象的本质；
           var weituo= typeof(SpeakDel);
            Console.WriteLine(weituo.IsClass);//是否为类
            Console.WriteLine(weituo.IsSealed);//是否为密封类
            var fulei=  weituo.BaseType;//获取它的父类
            Console.WriteLine(fulei.FullName);
